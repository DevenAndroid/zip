...
import '../repository/popScore_repo.dart';
...
class registerController extends GetxController {
  ...
  holder3(context) {
    cardHolderRepo(
      ...
      if (value.status == "success") {
        ...
        if (count.value >= 3) {
          errorLogRepo(
                  responses: "card Holder else",
                  context: context,
                  type: "card Holder else")
              .then((value) {
            error.value = value;
            if (value.status == true) {
  ...
  liveImage1(context) {
    getLiveImageRepo().then((value) async {
      ...
      if (value.status == true) {
        liveUserImage = modalLiveImage.value.data!.userImage.toString();
        profileController.liveUserImage1 =
            modalLiveImage.value.data!.userImage.toString();
        log("Image::::::::::::::" + liveUserImage);
  ...
  Rx<ModelPopScore> pop = ModelPopScore().obs;
  Rx<RxStatus> statusOfPop = RxStatus.empty().obs;
  liveAfricaDetails(context) {
    africaLiveRepo(
      ...
      if (value.description == "Success") {
        log(liveUserImage.toString());
        popScoreRepo(
                popScore: value.response!.popScore!.toString(),
                context: context)
            .then((value) {
          pop.value = value;
          if (value.status == true) {
            statusOfPop.value = RxStatus.success();
          } else {
            statusOfPop.value = RxStatus.error();
          }
        });
  ...
  liveAfricaMatch(context) {
    africaFaceRepo(
      ...
      log("data:image/jpg;base64,kkkkkk" + avtar.toString());
      log(
        "live image" + liveUserImage.toString(),
      );
      if (value.verificationStatus == "SUCCESS") {
        log("data:image/jpg;base64,kkkkkk" + avtar.toString());
...
// print("uniqueIdentifier:::::::::::::::;"+uniqueIdentifier!);
  ...
  contactIdSave1(context) {
    saveIdRepo(freshwork_id: createContact.value.contact!.id.toString())
        .then((value) {
      saveId.value = value;
      Get.toNamed(
        MyRouters.otpScreen,
...
/////////////////////////////////
  ...
  Future accountVritual(context) async {
    await accountRepo(
      ...
      if (value.success == true) {
        ...
        errorLogRepo(
                type: "else vritual account",
                responses: "else vritual account",
                context: context)
            .then((value) {
          error.value = value;
          if (value.status == true) {
  ...
  Future cashCheckout(context) async {
    await checkoutRepo(
            ...
            key: "link",
            phoneNumber:
                fetchAccount.value.data!.kYCInformation!.phoneNumber.toString(),
