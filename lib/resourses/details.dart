...
///////bridgeCard
  String bridgeCardApikey = "64529bd2bfdf28e7c18aa9da";
  ...
  Email(context) async {
    ...
    loginRepo(
      ...
      if (value.status == true) {
        ...
        Get.offAllNamed(MyRouters.loginPinScreen,
            arguments: [emailNoController.text.trim()]);
  ...
  forgot(context) async {
    forgotEmailRepo(
            context: context, email: emailForgotController.text.toString())
        .then((value) async {
      forgotEmail.value = value;
      if (value.status == true) {
        SharedPreferences pref = await SharedPreferences.getInstance();
        Get.to(() => const ForgotScreenOtp(),
            arguments: [emailForgotController.text.toString()]);
        // Get.offAllNamed(MyRouters.bottomNavbar);
  ...
  Login(context) {
    loginRepo(
      ...
      if (value.status == true) {
        SharedPreferences pref = await SharedPreferences.getInstance();
        pref.setString('cookie', value.authToken.toString());
        Get.offAllNamed(MyRouters.loginNoPinScreen,
            arguments: ["+234" + mobileNoController.text.trim()]);
        // Get.offAllNamed(MyRouters.bottomNavbar);
        statusOflogin.value = RxStatus.success();
        showToast(value.message.toString());
      } else {
